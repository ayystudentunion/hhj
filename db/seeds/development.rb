...
pekka_application = administrative_call.position_applications.create!(user: student_pekka, position: :position_member, personal_statement: "Olisin hyvä halloped!", custom: { 'degree' => 'YTM' })
administrative_call.position_applications.create!(user: student_paula, position: :position_deputy, member: pekka_application, personal_statement: "Haluaisin vain varajäseneksi siis.", custom: { 'degree' => 'HTM' })

recretional_group = helsinki.organs.create!(name: 'Huvitoimikunta', official: false)
recretional_group_call = recretional_group.calls.create!(title: 'Opiskelijaedustajia huvitoimikuntaan!', member_amount: 2, deputy_amount: 1, workflow: :workflow_proposal_available)
pekka_application = recretional_group_call.position_applications.create(user: student_pekka, position: :position_member, personal_statement: "Olisin hyvä huvittelija!")
recretional_group_call.position_applications.create(user: student_paula, position: :position_deputy, member: pekka_application, personal_statement: "Haluaisin vain varajäseneksi siis.")
...
unless Admin.where(email: 'halloped@localhost.fi').exists?

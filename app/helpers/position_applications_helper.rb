module PositionApplicationsHelper
  def applicant_name_with_coapplicant_for(application, show_modal_link = true)
    ...
    if application.deputy?
      text += ", #{translated_position('position_deputy')}: #{application.deputy.try(:name)}"
    elsif application.member?
      text += ", #{}: #{application.member.try(:name)}"
    end
    text + ')'
  end
  ...
  def translated_position(position)
    t("position_applications.show.#{position}.zero").downcase
  end
  ...
  def rearrange_pairs_adjacently(position_applications)
    ...
    pairs.delete_if(&:nil?).flatten | position_applications # | operation will append applications with no pair to the list
  end

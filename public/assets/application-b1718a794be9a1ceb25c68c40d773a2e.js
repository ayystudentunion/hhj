...
(function() {
  jQuery(function() {
    ...
    return $('#modal-wrap').on("click", container_class + " input[type=checkbox]", function() {
      var container, max, max_members_checked, message;
...
;
(function() {
  var initNavi, initOrganPage, initPopups, initRadioButtons, uniformify;

  initRadioButtons = function(delegateFor) {
    return delegateFor.delegate(".buttons .radio input", "change", function() {
      var buttons;
      buttons = $(this).parents('.buttons').first();
      buttons.find('.btn').removeClass('checked');
      return $(this).parents('.btn:first').addClass('checked');
    });
  };

  uniformify = function(node) {
    return $("input:checkbox, input:radio, input:file, input:text, textarea, submit", node).uniform();
  };

  initOrganPage = function(delegateFor) {
    var alignApplicants, alignCards, initAddMember, initCallSelectionDragNDrops, initDraggableCard, initDroppableSlot, initRemoveMember, initToggleArchived, initToolTips, sameContext, saveSelection;
    sameContext = function(droppable, draggable) {
      var draggableId, droppableId, getContextId;
      getContextId = function(elem) {
        var call;
        call = elem.parents('.call-for-application:first');
        if (call.length > 0) {
          return call.find('form.edit_call').attr('id');
        } else {
          return elem.parents('.organ-members').attr('class');
        }
      };
      droppableId = getContextId(droppable);
      draggableId = getContextId(draggable);
      return (droppableId != null) && (draggableId != null) && droppableId === draggableId;
    };
    alignCards = function(container) {
      container.find('.member-card:even').removeClass('no-margin');
      return container.find('.member-card:odd').addClass('no-margin');
    };
    alignApplicants = function(droppable) {
      return alignCards(droppable.parents('.call-for-application').find('.applicants'));
    };
    saveSelection = function(droppable, draggable) {
      var deputy, form, member;
      ...
      return superagent.post(form.attr('action')).type('form-data').send(form.serialize()).end(function() {
        return cached.clear();
      });
    };
    initDroppableSlot = function(elements) {
      return elements.droppable({
        activeClass: 'highlight-drop-area',
        accept: function(draggable) {
          if (!draggable.hasClass('member-card')) {
            return false;
          }
          if ($(this).find('.member-card').length > 0) {
            return false;
          }
          if ($(this).hasClass('no-deputy')) {
            return false;
          }
          return sameContext($(this), draggable);
        },
        drop: function(event, ui) {
          ...
          return alignApplicants(droppable);
        }
      });
    };
    initDraggableCard = function(elements) {
      return elements.draggable({
        revert: 'invalid'
      });
    };
    initToggleArchived = function() {
      var toggleArchived;
      toggleArchived = function(visible) {
        $('.archived-call-for-applications').toggle(visible);
        $('.hide-archived').toggle(visible);
        return $('.show-archived').toggle(!visible);
      };
      delegateFor.delegate(".show-archived", "click", function() {
        toggleArchived(true);
        return false;
      });
      return delegateFor.delegate(".hide-archived", "click", function() {
        toggleArchived(false);
        return false;
      });
    };
    initRemoveMember = function() {
      return $('.btn.remove-member').click(function() {
        var form;
        form = $(this).parents('form.edit_member');
        form.find('#member_current').val(false);
        superagent.post(form.attr('action')).type('form-data').send(form.serialize()).end(function() {
          return cached.clear();
        });
        $(this).parents('.member-card').fadeOut('fast', function() {
          var container;
          container = $(this).parents('.staff, .professors');
          $(this).remove();
          return alignCards(container);
        });
        return false;
      });
    };
    initAddMember = function() {
      return $('.add-member .btn').click(function() {
        var email, form, members;
        email = $(this).prev('input').val();
        form = $(this).parents('form.new_member');
        members = $(this).parents('.hallopeds, .staff, .professors');
        superagent.post(form.attr('action')).type('form-data').send(form.serialize()).end(function(response) {
          var findOrCreateNewRow, lastCard, newMemberRow, newSlot, placeHolder;
          findOrCreateNewRow = function() {
            var lastRow, newRow;
          ...
          if (members.hasClass('staff') || members.hasClass('professors')) {
    ...
    initCallSelectionDragNDrops = function() {
      ...
      $('.call-for-application.open .applicants').droppable({
        ...
        drop: function(event, ui) {
          ...
          return saveSelection(droppable, ui.draggable);
        },
        accept: function(draggable) {
          if (!draggable.hasClass('member-card')) {
            return false;
      ...
      return initDroppableSlot($('.call-for-application.open .member-card-empty, .edit-members .member-card-empty'));
    };
    initToolTips = function() {
      return $('.not_eligible').qtip({
        position: {
          at: 'bottom left'
        }
      });
  ...
  initPopups = function(delegateFor) {
    return delegateFor.delegate("a.popup", "click", function() {
  ...
  initNavi = function() {
    var i, isActive, len, li, ref, results;
    isActive = function(a) {
      return (window.location.pathname === a.attr('href')) || (window.location.pathname.indexOf(a.attr('href')) === 0 && !a.hasClass('home'));
    };
    ref = $('.navi-links li');
    results = [];
    for (i = 0, len = ref.length; i < len; i++) {
      li = ref[i];
      results.push($(li).toggleClass('active', isActive($(li).find('a'))));
    }
    return results;
  };

  window.initDom = function(selector) {
    var node;
    node = $(selector);
    uniformify(node);
...
(function() {
  var ERROR, Future, SUCCESS, WAITING,
    bind = function(fn, me){ return function(){ return fn.apply(me, arguments); }; };
  ...
  Future = (function() {
    function Future() {
      ...
      this.fulfill = bind(this.fulfill, this);
    ...
    Future.prototype.callback = function(error, value) {
      if (error != null) {
    ...
    Future.prototype.fail = function(error) {
      var callback, i, len, ref;
      if (this.status !== WAITING) {
        throw "Diverging future";
      }
    ...
    Future.prototype.get = function(callback) {
      switch (this.status) {
        case WAITING:
        ...
        case ERROR:
          return callback(this.result, null);
      }
    };
...
(function() {
  var choose_org, init_modals;

  this.open_modal_dialog = function(url) {
    var wrap;
    wrap = $('#modal-wrap').empty().fadeIn('fast');
    cached.clear();
    return cached.get(url + ".fragment", function(error, data) {
      var org_path;
      $(data).appendTo(wrap);
      initDom(wrap);
      ...
      if (org_path) {
        if (org_path) {
          return async.mapSeries(org_path.split('|'), choose_org(wrap), (function() {}));
        }
      }
    });
  };

  choose_org = function(parent) {
    return function(id, cb) {
      var option, select;
      option = parent.find("option[value='" + id + "']");
      select = option.parent();
      select.unbind("resolved").bind("resolved", function() {
        return cb();
      });
  ...
  init_modals = function() {
    $('a.js-modal').live('click', function() {
      var href;
      href = $(this).attr('href');
      open_modal_dialog(href);
      return false;
    });
    $('#modal-wrap').delegate('.close-modal', 'click', function() {
      $('#modal-wrap').fadeOut('fast');
      if ($(this).attr('href') === '#') {
        return false;
      }
    });
    $('#modal-wrap').delegate('.js-submit', 'click', function(e) {
      var form;
    ...
    $("#modal-wrap").delegate(".lang.btn", "click", function() {
      var changeModalFormLanguage;
      changeModalFormLanguage = function(forms_container, locale) {
        ...
        return forms_container.find(".lang.btn[hreflang='" + locale + "']").toggleClass('active', true);
      };
      changeModalFormLanguage($(this).parents('.content:first'), $(this).attr('hreflang'));
      return false;
    });
    $("#modal-wrap").delegate("select.organizations", "change", function() {
      var organization_id, select;
      select = $(this);
      organization_id = select.val();
      select.nextAll().remove();
      $.get($("#modal-wrap .modal").attr('data-url'), function(organizations) {
        var children, default_option, template;
        children = _(organizations).filter(function(org) {
          return org.parent_id === organization_id;
        });
        ...
        return select.trigger("resolved");
      });
      return false;
    });
    $("#modal-wrap").delegate("#send-application .radio input", "change", function(e) {
      $(this).parents('.item-row:first').find("#position_application_deputy_of").toggle($(this).val() === 'position_deputy');
      return event.preventDefault();
    });
    $("#modal-wrap").delegate("#send-application-for-administrational-call .radio input", "change", function(e) {
      var i, len, pair_select, pair_selects, shown_id;
      pair_selects = $(this).parents('.item-row:first').find(".pair-select");
      shown_id = "show_when_" + ($(this).val()) + "_selected";
      for (i = 0, len = pair_selects.length; i < len; i++) {
        pair_select = pair_selects[i];
        $(pair_select).toggle(shown_id === $(pair_select).attr('id'));
      }
      return event.preventDefault();
    });
    $("#modal-wrap").delegate("#send-application-for-administrational-call  .email-reminder-fields input[type=checkbox]", "change", function() {
      return $(this).parents('.email-reminder-fields').find('input[type=text]').toggle();
    });
    return $("#modal-wrap").delegate("#new-alliance .call select", "change", function() {
      $('.applications_lists .applications_for_one_call').hide();
      $('.applications_lists input:checkbox').prop('checked', false);
      $.uniform.update();
      return $("#applications-for-call-" + $(this).val()).show();
    });
  };
...
(function() {
  var initLanguageSelectors;

  initLanguageSelectors = function() {
    return $('.language-sel a').click(function() {
...
(function() {
  var init_routing, loadFragment, navigate, testEnvironment;

  loadFragment = function(locale, university, controller, param, modal) {
    var contentWrap, getParams, rest, url;
    if (controller == null) {
      controller = "";
    }
    if (param == null) {
    ...
    cached.getDOM(url, function(error, dom) {
      return initDom(dom.clone().appendTo(contentWrap.empty()));
    });
    return true;
  };

  testEnvironment = function() {
    return $('body').attr('data-env') === 'test';
  };

  init_routing = function() {
    if (!History.enabled || testEnvironment()) {
      return;
    }
    ...
    $('a').live('click', function(ev) {
      ...
      $this = $(this);
      href = $this.attr('href');
      if ($this.hasClass('js-modal' || href === "#")) {
        return false;
      }
      if (href.indexOf('http://') === 0 || $this.hasClass("js-reload")) {
        return true;
      } else {
        navigate(href);
        return false;
    ...
    return History.Adapter.bind(window, 'statechange', function() {
      return navigate(History.getState().hash);
    });
  };
